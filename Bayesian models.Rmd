...
model_OD3 <- ordinal::clmm2(behaviour ~ time_elapsed_r + intervention + Ext_temp_celsius,
                          random = 1+Address, data = df, Hess=TRUE, nAGQ = 1)
summary(model_OD3)
...
#Attempt multi-nomial mixed effect model
library(mixcat)
npmlt(df$behaviour ~ df$time_elapsed_r + df$intervention*df$Site + df$Ext_temp_celsius, random=~1+df$time_elapsed_r, id = df$Address)

```

## Survey data
```{r}
lmer(temp.c ~ time_elapsed_r + intervention*Site + Ext_temp_celsius + (time_elapsed_r|Address), data = df)
