...
M <- cs %>% select(avg_sp_overall_excdfltCL, 'Political.orientation', PEB_activist, PEB_pragmatist, 'AwarenessConsequences', 'AscriptionResponsibility', 
...
corrplot::corrplot(M, method = "number", type = "lower", order = "AOE")
...
# corrplot::corrplot.mixed(M, order = "hclust",lower.col = "white", number.cex = .5, tl.pos = "lt", tl.cex = .7, tl.col = colcode)

corrplot::corrplot.mixed(M, order = "hclust",lower.col = "white", number.cex = .5, tl.pos = "lt", tl.cex = .7)
...
df_xd %>% ggplot(aes(x=temp.c, fill=intervention)) + geom_density(adjust=1.5, alpha=.3) + xlim(-5,5) + facet_wrap(~ Site) + ggtitle("Data excluding default settings") + scale_fill_discrete(name = "Intervention") +
    theme_ipsum(plot_title_size = 9, plot_title_face = "plain", strip_text_size = 8) + labs(x = "Temperature difference from default") + theme(axis.title.y = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
#these variables seem to have no effect - test for interaction effects with intervention

m1 <- lmer(temp.c ~ 1 + NEP_mean_sz*intervention + time_elapsed_r + external.z + intervention * Site + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
m1 <- lmer(temp.c ~ 1 + PEB_activist_sz*intervention + time_elapsed_r + external.z + intervention * Site + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
m1 <- lmer(temp.c ~ 1 + PEB_pragmatist_sz*intervention + time_elapsed_r + external.z + intervention * Site + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
m1 <- lmer(temp.c ~ 1 + Habit_sz*intervention + time_elapsed_r + external.z + intervention * Site + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
m1 <- lmer(temp.c ~ 1 + SocialNorm_sz*intervention + time_elapsed_r + external.z + intervention * Site + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
...
# Effect size of moralisation on exc default data
...
# DV = temp setting; IV = moralisation; mediator = intention
med1 <- psych::mediate('temp.c', 'moralisation.z', m = c('Intention_sz'), data = df_survey)
med1
mediate.diagram(med1, main = "c = Direct Effect; c' = Indirect Effect", xlim = c(0,10), ylim = c(2,9), l.cex = .7)

# DV = temp setting; IV = ascription; mediator = intention
med2 <- psych::mediate('temp.c', 'AscriptionResponsibility_sz', m = c('Intention_sz'), data = df_survey)
med2
mediate.diagram(med2, main = "c = Direct Effect; c' = Indirect Effect", xlim = c(0,10), ylim = c(2,9), l.cex = .7)

# DV = temp setting; IV = PBC; mediator = intention
med3 <- psych::mediate('temp.c', 'PBC_sz', m = c('Intention_sz'), data = df_survey)
med3
mediate.diagram(med3, main = "c = Direct Effect; c' = Indirect Effect", xlim = c(0,10), ylim = c(2,9), l.cex = .7)

# DV = temp setting; IV = awareness consequences; mediator = intention
med4 <- psych::mediate('temp.c', 'AwarenessConsequences_sz', m = c('Intention_sz'), data = df_survey)
med4
mediate.diagram(med4, main = "c = Direct Effect; c' = Indirect Effect", xlim = c(0,10), ylim = c(2,9), l.cex = .7)


# DV = temp setting; IV = social norm; mediator = intention
med5 <- psych::mediate('temp.c', 'SocialNorm_sz', m = c('Intention_sz'), data = df_survey)
med5
mediate.diagram(med5, main = "c = Direct Effect; c' = Indirect Effect", xlim = c(0,10), ylim = c(2,9), l.cex = .7)
...
#refit models with interaction effect
m1 <- lmer(temp.c ~ 1 + time_elapsed_r + external.z + intervention * Site * AwarenessConsequences_sz + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
plot_model(m1, type = "int")
m1 <- lmer(temp.c ~ 1 + time_elapsed_r + external.z + intervention * Site * AscriptionResponsibility_sz + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
plot_model(m1, type = "int")
m1 <- lmer(temp.c ~ 1 + time_elapsed_r +external.z + intervention * Site * PBC_sz + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
plot_model(m1, type = "int")
m1 <- lmer(temp.c ~ 1 + time_elapsed_r +external.z + intervention * Site * moralisation.z + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
plot_model(m1, type = "int")
m1 <- lmer(temp.c ~ 1 + time_elapsed_r + external.z + intervention * Site * Intention_sz + (time_elapsed_r|site_room),data=df_survey)
summary(m1)
plot_model(m1, type = "int")
