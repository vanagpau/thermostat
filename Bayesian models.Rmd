...
survey <-left_join(cs, irus_data %>% filter(site_room %in% c(cs$site_room)) %>% filter (exclude == 1) %>%
...
write_csv(survey, file = "SurveyData.csv")
```

## QUICK IMPORT AND CREATE DATA-FRAME (df) for modelling
```{r}

#Import csv's
irus_data <- read_csv("IrusData.csv")
...
df <- irus_data %>%
  ...
  mutate(temp.c = (Setpoint - mean(Setpoint,  na.rm=T))/sd(Setpoint, na.rm=T)) %>% #standardise set-point temperature
...
#Exclude temp = 19 default values
...
df %>% ggplot() + geom_histogram(aes(x = temp.c), binwidth = .5) 
df_xd %>% ggplot() + geom_histogram(aes(x = temp.c), binwidth = .5) 

# Plot mean temp setting over time
df %>% ggplot(aes(y = temp.c)) + geom_histogram() + facet_wrap(df$intervention) + ylim(-2,5) + xlim(0,2.5e5)
df_xd %>% ggplot(aes(y = temp.c)) + geom_histogram() + facet_wrap(df_xd$intervention) + ylim(-2,5) + xlim(0,2e4)


# Mixed-model to model how temp.c depends on time as a random effect does not converge
mm1 <- lmer(temp.c ~ (time_elapsed|Address), data = df)
mmxd1 <- lmer(temp.c ~ (time_elapsed|Address), data = df_xd)

# checks at https://rstudio-pubs-static.s3.amazonaws.com/33653_57fc7b8e5d484c909b615d8633c01d51.html

#check singularity - not a problem
tt <- getME(mm1,"theta")
ll <- getME(mm1,"lower")
min(tt[ll==0])

# change optimisation algorithm - still does not converge
mm1 <- lmer(temp.c ~ (time_elapsed|Address), data = df, control = lmerControl(optimizer = "bobyqa"))
...
# So let's try some other models (including defaults)
...
#fails to converge degenerate Hessian negaitve eigenvalues and unable to evaluate gradient - too complex?
mm8 <- lmer(temp.c ~ time_elapsed_r + intervention*Site + Ext_temp_celsius + (time_elapsed_r+intervention|Address) + (time_elapsed_r+intervention|Site), data = df, control = lmerControl(optimizer = "bobyqa"))
summary(mm8)
...
bm3 <- brm(formula = temp.c ~ intervention + (1+time_elapsed|Address),
          data = df, cores = 4, warmup = 1500, iter = 4000,
          file=("bm3"), control = list(adapt_delta = 0.95),
          file_refit = getOption("brms.file_refit", "on_change"))
