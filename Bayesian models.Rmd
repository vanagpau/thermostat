...
#For Reading virtual machine setwd("/home/users/kl819459/Desktop/thermostat")
...
df <- irus_data %>%
  ...
  mutate(time_elapsed = date_time - date_time[1]) %>% #zero time clock
...
#Replace "Before" and "After" with 0's and 1's in df$intervention variable
df$intervention <- ifelse(df$intervention == "Before",0,1)
...
# Dependent variable is 'Setpoint'
...
dat_sample <- list(S = df_sample$Setpoint,
...
#Plot posterior distribution savey wavey


#Model 2 - SAMPLE data; depends on participant
flist <- alist (S ~ dnorm(mu, sigma),
                mu <- a + bP[P],
                a ~ dnorm(mu_P, sigma_P),
                mu_P ~ dnorm(19,0),
                sigma_P ~dexp(1),
                bP ~ dnorm(0, 0.5),
                sigma ~ dexp(1))

m1 <- ulam(flist, data = dat_sample, chains = 2, cores = 2)
