...
df$intervention <- ifelse(df$intervention == "Before",1,2)
...
#m3 <- ulam(flist, data = dat, chains = 2, cores = 2)

# Fit Adam model using BRMS

m4 <- brm(formula = temp.c ~ time_elapsed + (1+time_elapsed|Address),
          data = df_sample, family = gaussian(),
          prior = set_prior("normal(0,0.5)", class = "b"),
          warmup = 1000, iter = 2000, chains = 4,
          control = list(adapt_delta = 0.95))

m5 <- brm(formula = temp.c ~ time_elapsed + (1+time_elapsed|Address),
          data = df_sample, family = gaussian(),
