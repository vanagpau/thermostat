...
# Mixed-model to model how temp.c depends on time as a random effect
mm1 <- lmer(temp.c ~ (1+time_elapsed|Address), data = df)
...
bm1 <- brm(formula = temp.c ~ (1+time_elapsed|Address), data = df,
           warmup = 1000, iter = 3000, cores = 4, control = list(adapt_delta = 0.95),
           file=("bm1"), file_refit = getOption("brms.file_refit", "on_change"))

#Modelling manipulation effect
lm1 <- lm(temp.c ~ intervention, data = df)
lm2 <- lm(temp.c ~ time_elapsed + intervention, data = df)

#need to re-run this model as accidently over-saved bm2 previously
